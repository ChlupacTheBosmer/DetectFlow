...
def validate_and_process_input(input_data, s3_manipulator, s3_validator, manipulator):
    if isinstance(input_data, str):
        return process_single_input(input_data, s3_manipulator, s3_validator, manipulator)
    elif isinstance(input_data, (list, tuple)):
        return process_multiple_inputs(input_data, s3_manipulator, s3_validator, manipulator)
    else:
        raise ValueError("Invalid input format")

def process_single_input(input_data, s3_manipulator, s3_validator, manipulator):

    if s3_validator.is_s3_bucket(input_data):
    ...
    elif s3_validator.is_s3_directory(input_data):
        # Fetch directories from the S3 directory
        bucket_name, prefix = s3_validator._parse_s3_path(input_data)
        return s3_manipulator.list_directories_s3(bucket_name, prefix, full_path=True), (
            True, False, False, False)

    elif s3_validator.is_s3_file(input_data):
...
def process_multiple_inputs(input_data, s3_manipulator, s3_validator, manipulator):
    directories = []
    flags = (False, False, False, False)  # default flags
    for item in input_data:
        processed_item, item_flags = process_single_input(item, s3_manipulator, s3_validator, manipulator)
